...
class Service extends Model
{
    ...
    public static function getList()
    {
        ...
        $list = self::order('service_id','desc')->where($where)->paginate($limit)->each(function($item)use($action, $types){
            ...
            $item['slove_question'] = $item['slove_question']>0?$types[$item['slove_question']]:'不展示';
            return $item;
        });
