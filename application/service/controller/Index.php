...
class Index extends Base
{
    ...
    public function menu(){
        ...
        if($login['level'] == 'super_manager'){
            ...
            if( $login['slove_question'] > 0 ){
                $menu = AdminPermission::table('wolive_admin_permission')->where($where)->whereOr(['id'=>7])->order('sort','asc')->select();
            }else{
                $menu = AdminPermission::table('wolive_admin_permission')->where($where)->order('sort','asc')->select();
            }
        }
    ...
    public function service()
    {
        ...
        // $groupId = Admins::table("wolive_queue")->where("visiter_id", $visiter_id)->order("qid desc")->limit(1)->value("groupid");

        // $res = Admins::table('wolive_service')->where('business_id', "{$business_id}")->where('frozen', 0)->where('groupid', "{$groupId}")->where('service_id', '<>', $login['service_id'])->select();
        //jg改
                $res = Admins::table('wolive_service')->where('business_id', "{$business_id}")->where('frozen', 0)->where('service_id', '<>', $login['service_id'])->select();
    ...
    public function getswitch()
    {
        ...
        $result = Admins::table('wolive_queue')->where("visiter_id", $post['visiter_id'])->where('business_id', $login['business_id'])->update(['service_id' => $post['id']]);
