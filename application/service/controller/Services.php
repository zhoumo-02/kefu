...
class Services extends Base
{
    ...
    {
        $post = $this->request->post();
        $result = Service::where('service_id', $post['service_id'])->update(['frozen' => $post['frozen']]);
        if ($result) $this->success('操作成功！');
        $this->error('操作失败！');
    }
    
    public function slove_question()
    {
        $post = $this->request->post();
        $result = Service::where('service_id', $post['service_id'])->update(['slove_question' => $post['slove_question']]);
        if ($result) $this->success('操作成功！');
        $this->error('操作失败！');
    }
