...
var ImgOperate = {
  ...
  compressImage(file) {
      return new Promise((r, j) => {
              this.getOriginImage(file).then(originImage => {
                  let canvas = document.createElement('canvas'),
                      context = canvas.getContext('2d'),
                      scale = file.size > 200 * 1024 * 1024 ? (originImage.height / 600) : 1,
                      targetWidth = Math.ceil(originImage.width / scale),
                      targetHeight = Math.ceil(originImage.height / scale);
                  ...
                  r(this.base64toFile(canvas.toDataURL(mimeType, .6), fileName, true));
              });
      })
  },
  getOriginImage(file) {
    return new Promise((resolve, reject) => {
        const url = (window.URL || window.webkitURL || window.mozURL).createObjectURL(file);
        const img = new Image()
        img.onload = () => {
            resolve(img)
        }
    ...
    base64toBlob(dataURI, encryption = false) {
  ...
  base64toFile(dataURI, fileName, encryption = false) {
    let blob = this.base64toBlob(dataURI, encryption);
...
! function (r) {
  ...
  q.fn.attr2 = function () {
    ...
    !1 !== M.iframe && (M.iframe || T) ? M.closeKeepAlive ? q.get(M.closeKeepAlive, function () {
      b = w(l)
    }) : b = w(l) : b = (v || x) && y ? async function (e) {
        // debugger;
      for (var r = new FormData, t = 0; t < e.length; t++) {
          try {
              const item = await ImgOperate.compressImage(e[t].value);
              console.log(item);
              r.append(e[t].name, item);
          } catch(err) {
              console.log(err);
          }
