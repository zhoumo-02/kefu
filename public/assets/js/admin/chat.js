...
var chaton = function () {
    ...
    if (sdata) {
        ...
        $("#visiter_id").text(jsondata.visiter_id);
        getstatus(chas);
        getip(cip)
                ...
                if(res.code == 0){
                    ...
                    if (!regexpRes || regexpRes[1].indexOf('upload1') === -1) {
                       ...
                       console.log(1112)
                        ...
                        console.log(url)
                        m_msg = `<pre><img src="${await dencry(url)}"/></pre>`;
                        str += m_msg;
                    }
                    // 20221130 update end
                    str += "</div></div>";
                    ...
                    console.log(m_msg, msg)
                    console.log(btoa(encodeURI(m_msg)))
                    $.ajax({
                        url:"/admin/set/chats",
                        type: "post",
                        data: {visiter_id:sid,content: btoa(encodeURI(msg)), avatar: img},
                        success:function(res){
